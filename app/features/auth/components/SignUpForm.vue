<template>
    <el-form ref="formRef" :model="form" :rules="rules" @submit.prevent="submit"
        ...
        <div class="flex items-center gap-x-3">
            ...
            <span class="text-xs text-slate-400 font-medium">or sign up with email</span>
            <span class="flex-1 h-px bg-slate-200" />
        </div>
        ...
        <div class="space-y-4">
            ...
            <!-- Role -->
            <div class="space-y-1.5">
                <span class="block text-sm font-medium text-slate-700">I am a...</span>
...
// Extend email rules with a server-side error slot so 409 renders inline
const rules = computed<FormRules>(() => ({
    ...registerRules,
    email: [
        ...(registerRules.email as FormItemRule[]),
        {
            validator: (_rule, _val, callback) => {
                if (serverEmailError.value) callback(new Error(serverEmailError.value))
                else callback()
            },
            trigger: 'change',
        },
...
watch(() => form.email, () => { serverEmailError.value = '' })
...
const submit = async () => {
    ...
    try {
        ...
        if (status === 409) {
            serverEmailError.value = 'An account with this email already exists.'
            await nextTick()
