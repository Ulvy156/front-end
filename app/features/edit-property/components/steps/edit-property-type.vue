...
const { t } = useI18n()
const selected = ref(props.form.propertyType || 'house')
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 rounded-(--radius) p-5 shadow-md">
    ...
    >
      ...
      >
        <BaseIconClient :name="item.icon" :size="28" />
      </div>

      <div class="text-lg font-semibold text-gray-800">{{ item.label }}</div>
      <div class="text-sm text-gray-500 mt-1">{{ item.sub }}</div>
    </div>
  </div>
</template>
