  ...
  <div class="bg-white rounded-2xl border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-3 gap-3 mb-3">
          <div>
            ...
            <input v-model="form.parkingDetails[pk].slots" type="number" min="0"
              ...
              :placeholder="t('post_property.amenities.slots_placeholder')" @input="console.log('Parking slots for', pk, ':', form.parkingDetails[pk].slots)" />
          </div>
        ...
        <div v-if="!form.parkingDetails[pk]?.isFree" class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input v-model="form.parkingDetails[pk].price" type="number" min="0"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 outline-none focus:border-emerald-500 focus:ring-3 focus:ring-emerald-500/15 transition"
              :placeholder="t('post_property.amenities.price_placeholder')" @input="console.log('Parking price for', pk, ':', form.parkingDetails[pk].price)" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <textarea v-model="form.additionalNotes" rows="3"
        ...
        placeholder="Any other rules, expectations, or information renters should know..." @input="console.log('Additional notes input:', form.additionalNotes)"></textarea>
    </div>
  </div>
</template>
