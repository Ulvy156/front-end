  ...
  <div class="flex min-h-screen">
    ...
    <aside class="fixed left-0 top-0 h-screen w-60 bg-white border-r border-gray-200 flex flex-col z-30">
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        ...
        <AdminNavItem to="/admin" icon="layout-dashboard" :label="$t('admin.nav.dashboard')" :exact="true" />

        <!-- PEOPLE -->
        <p class="admin-section-label">{{ $t('admin.nav.people') }}</p>
        <AdminNavItem to="/admin/users" icon="users" :label="$t('admin.nav.users')" />
        <AdminNavItem to="/admin/landlords" icon="user-check" :label="$t('admin.nav.landlords')" />
        ...
        <AdminNavItem
          ...
          :badge="pendingCount"
        />
        <AdminNavItem to="/admin/properties" icon="building" :label="$t('admin.nav.allProperties')" />
        <AdminNavItem to="/admin/featured" icon="star" :label="$t('admin.nav.featured')" />

        <!-- CONFIGURATION -->
        <p class="admin-section-label">{{ $t('admin.nav.configuration') }}</p>
        <AdminNavItem to="/admin/property-types" icon="layers" :label="$t('admin.nav.propertyTypes')" />
        <AdminNavItem to="/admin/amenities" icon="sparkles" :label="$t('admin.nav.amenities')" />
        <AdminNavItem to="/admin/house-rules" icon="shield-check" :label="$t('admin.nav.houseRules')" />
      ...
      <div class="border-t border-gray-200 p-3 space-y-0.5 shrink-0">
        <AdminNavItem to="/admin/settings" icon="settings" :label="$t('admin.nav.settings')" />
        <button
...
import AdminNavItem from '~/features/admin/components/AdminNavItem.vue'
